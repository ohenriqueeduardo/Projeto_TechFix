      ...
      <aside className="hidden lg:flex flex-col w-24 hover:w-80 border-r border-foreground/5 bg-card/30 sticky top-0 h-screen transition-all duration-300 group/sidebar z-30 overflow-x-hidden no-scrollbar">
        ...
        <nav className="flex-1 px-4 py-8 space-y-2 select-none overflow-y-auto no-scrollbar flex flex-col items-center group-hover/sidebar:items-stretch">
          ...
          {menuItems.map((item) => {
            ...
            return (
              <Link
                key={item.path}
                to={item.path}
                className={`flex items-center gap-0 group-hover/sidebar:gap-4 w-14 group-hover/sidebar:w-full h-14 rounded-2xl transition-all duration-300 group relative overflow-hidden justify-center group-hover/sidebar:justify-start px-0 group-hover/sidebar:px-5 shrink-0 ${isActive
          ...
          <div className="pt-10">
            ...
            <Link
              to="/cliente/ajuda"
              className={`flex items-center gap-0 group-hover/sidebar:gap-4 w-14 group-hover/sidebar:w-full h-14 rounded-2xl transition-all duration-300 justify-center group-hover/sidebar:justify-start px-0 group-hover/sidebar:px-5 shrink-0 ${location.pathname === '/cliente/ajuda'
        ...
        <header className="h-24 border-b border-foreground/5 bg-background/50 backdrop-blur-xl flex items-center justify-between px-10 sticky top-0 z-40">
          <h2 className="font-bold text-2xl hidden md:block bg-clip-text text-transparent bg-gradient-to-r from-foreground to-foreground/80 animate-in fade-in duration-300">
            {(() => {
              ...
              if (path.includes('/profissional/chat')) return 'Chat & Mensagens';
