...
const ClientLayout = () => {
  ...
  return (
    <div className="min-h-screen flex bg-[#070B14]">
      {/* Sidebar Desktop */}
      <aside className="hidden lg:flex flex-col w-64 border-r border-white/5 bg-card/30 sticky top-0 h-screen">
        <div className="p-6">
      ...
      <div className="flex-1 flex flex-col min-w-0">
        <header className="h-16 border-b border-white/5 bg-background/50 backdrop-blur-md flex items-center justify-between px-4 md:px-8 sticky top-0 z-40">
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="flex items-center gap-3 pl-4 border-l border-white/5">
              ...
              <img src="https://i.pravatar.cc/150?u=sofia" className="w-8 h-8 rounded-full border border-white/10" alt="Avatar" />
            </div>
          </div>
        </header>
        ...
        {/* Mobile Nav */}
        <nav className="lg:hidden fixed bottom-0 left-0 w-full bg-card/80 backdrop-blur-lg border-t border-white/5 flex justify-around items-center h-16 px-2 z-50">
          {menuItems.map((item) => (
            <Link 
              key={item.path} 
              to={item.path}
