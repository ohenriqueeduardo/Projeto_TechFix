          ...
          <div className="md:hidden absolute top-16 left-0 w-full bg-background border-b border-foreground/5 p-6 flex flex-col gap-4 animate-in slide-in-from-top duration-300 z-50">
            {user ? (
              <>
                ...
                {user.role === 'admin' && (
                  <>
                    <Link to="/admin/perfil" className="flex items-center gap-3 text-lg font-bold py-2" onClick={() => setIsMenuOpen(false)}>
                      <UserIcon className="w-5 h-5 text-primary" /> Perfil Admin
                    </Link>
